...
- `activity_main.xml`는 제거되었고, 설정 UI는 Compose 코드로만 구성된다.
...
- AppCompat 의존성은 제거된 상태이며, UI 표준은 Compose Material 3 기준으로 유지한다.
- Compose 상태 수집은 `collectAsStateWithLifecycle` 대신 `collectAsState`를 사용한다.
  - 이 환경에서 lifecycle-compose 조합 이슈 가능성을 줄이기 위한 안정화 선택.
...
- Compose `Scaffold`와 시스템 인셋 처리(`WindowInsets`)를 함께 사용 중이므로, 상단 여백/겹침 변경은 실제 기기에서 재확인한다.
...
- 현재 프로젝트는 테스트 소스/테스트 의존성을 제거한 상태다.
  - 테스트를 다시 도입할 때 `build.gradle.kts`의 test/androidTest 설정을 함께 복구해야 한다.
- Manifest의 `dataExtractionRules`/`fullBackupContent` 참조는 제거되었다.
  - 앱 백업 정책이 필요해지면 XML 규칙 파일을 명시적으로 다시 추가한다.
